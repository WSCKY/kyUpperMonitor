...
public class kyLinkPackage {
	/* file data cache size */
	public static final int MAIN_DATA_CACHE = 256;
	...
	public kyLinkPackage(int cache_size) {
		stx1 = kySTX1;
		stx2 = kySTX2;
		dev_id = 0;
		msg_id = 0;
	...
	public kyLinkPackage copy() {
		kyLinkPackage cpy = new kyLinkPackage(this.length);
		cpy.dev_id = this.dev_id;
		cpy.msg_id = this.msg_id;
		cpy.length = this.length;
		System.arraycopy(this.rData, 0, cpy.rData, 0, this.length);
		cpy.crc = this.crc;
		return cpy;
	}
}
